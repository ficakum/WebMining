...
# Initialize RandomForestRegressor models for latitude and longitude
...
# Fit the models
rf_latitude.fit(X_train_scaled, y_latitude_train)
rf_longitude.fit(X_train_scaled, y_longitude_train)
...
predicted_latitude = rf_latitude.predict(scaler.transform(new_data_point))
predicted_longitude = rf_longitude.predict(scaler.transform(new_data_point))

# Calculate R2 scores for latitude and longitude on the test set
predicted_latitude_test = rf_latitude.predict(X_test_scaled)
predicted_longitude_test = rf_longitude.predict(X_test_scaled)

r2_latitude = r2_score(y_latitude_test, predicted_latitude_test)
print(f'R2 Score for Latitude: {r2_latitude}')

r2_longitude = r2_score(y_longitude_test, predicted_longitude_test)
print(f'R2 Score for Longitude: {r2_longitude}')
...
print(f'Mean Squared Error for Latitude: {mse_latitude}')

mse_longitude = mean_squared_error(y_longitude_test, predicted_longitude_test)
print(f'Mean Squared Error for Longitude: {mse_longitude}')
...
plt.title('Latitude and Longitude Prediction with Random Forest')
plt.legend()
plt.show()

# Visualize feature importance for latitude
plt.figure(figsize=(10, 6))
plt.barh(X.columns, rf_latitude.feature_importances_)
...
plt.figure(figsize=(10, 6))
plt.barh(X.columns, rf_longitude.feature_importances_)
plt.xlabel('Feature Importance')
plt.ylabel('Features')
